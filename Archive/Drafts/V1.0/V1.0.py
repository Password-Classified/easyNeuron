...
class Layer_Dense(object):
    ...
    def forward(self, inputs):
        ...
        for neuron in range(len(self.biases)):  # iterate for the num of neurons
            ...
            self.output.append(dotted + self.biases[neuron])
        return self.output
...
class Optimizer_GradDesc(Optimizer):
    ...
    def optimize(self, inputs, targets, reps=1000, epoch=10000):
...
if __name__ == '__main__':  # Will not run if imported as a module
    ...
    l1 = Layer_Dense(2, 1)
    l2 = Layer_Dense(1, 1)
    grad = Optimizer_GradDesc([[l1, ReLU],
                               [l1, sig]], ce)

    grad.optimize(training_data, training_targets, reps=10)

    pred = sig.forward(l1.forward(mystery_flower_red))
    if pred[0] > 0.5:
        col = 'blue'
    elif pred[0] < 0.5:
        col = 'red'
    ...
    pred = sig.forward(l1.forward(mystery_flower_blue))
    if pred[0] > 0.5:
        col = 'blue'
    elif pred[0] < 0.5:
        col = 'red'
